...
- Etag
  - [x] Stabilize manifest ETag generation with content hashing (server `transform.ts`).
  - [x] Normalize `If-None-Match` handling and reuse across endpoints.
  - [x] Extend ETag/304 responses to `/v1/providers`, `/v1/providers/:providerId/models`, and `/v1/version`.
  - [x] Update documentation examples to reflect strong ETags and endpoint caching behavior.
